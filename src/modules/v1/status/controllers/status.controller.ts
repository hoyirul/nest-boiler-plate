 ...
 * src/modules/v1/status/controllers/status.controller.ts
 */
...
import { StatusUseCase } from "@/modules/v1/status/usecases/status.usecase";
import * as statusTypes from "@/modules/v1/status/domains/status.types";
import { ListQueryDTO } from "@/modules/v1/status/domains/status.dto";
...
export class StatusController {
  private readonly logger = Loggers.status;
  constructor(private readonly uc: StatusUseCase) {}
  ...
  async list(
    ...
    return ResponseTrait.success({
      ...
      message: getMessage(lang, 'api.modules.status.fetched'),
      httpCode: HTTP.OK,
  ...
  async create(
    @Body(new ZodValidationPipe(statusTypes.CreateStatusSchema)) 
    body: statusTypes.CreateStatusDTO, 
    @Lang() lang: string
  ) {
    ...
    return ResponseTrait.success({
      module: MODULE.STATUS,
      statusLabel: RESP_STATUS.OK,
      message: getMessage(lang, 'api.modules.status.created'),
      httpCode: HTTP.CREATED,
      data,
    });
  }
  ...
  async update(
    @Param('id') id: number, 
    @Body(new ZodValidationPipe(statusTypes.UpdateStatusSchema)) 
    body: statusTypes.UpdateStatusDTO, 
    @Lang() lang: string
  ) {
    ...
    return ResponseTrait.success({
      module: MODULE.STATUS,
      statusLabel: RESP_STATUS.OK,
      message: getMessage(lang, 'api.modules.status.updated'),
      httpCode: HTTP.OK,
      data,
    });
  }
  ...
  async delete(@Param('id') id: number, @Lang() lang: string) {
    ...
    return ResponseTrait.success({
      module: MODULE.STATUS,
      statusLabel: RESP_STATUS.OK,
      message: getMessage(lang, 'api.modules.status.deleted'),
      httpCode: HTTP.OK,
    });
  }
  ...
  async restore(@Param('id') id: number, @Lang() lang: string) {
    ...
    return ResponseTrait.success({
      module: MODULE.STATUS,
      statusLabel: RESP_STATUS.OK,
      message: getMessage(lang, 'api.modules.status.restored'),
      httpCode: HTTP.OK,
    });
  }
}

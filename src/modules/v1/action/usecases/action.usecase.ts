 ...
 * src/modules/v1/action/usecases/action.usecase.ts
 */

import { ActionRepository } from "@/modules/v1/action/repositories/action.repository";
...
export class ActionUseCase {
  ...
  async create(payload: CreateActionDTO) {
    ...
    await db.transaction(async (tx: unknown): Promise<ReturnType<typeof this.repo.create>> => {
      ...
      if (isExist) {
        throw ValidationError("api.common.validation_failed", {
          name: "api.modules.action.already_exists",
        });
      }
  ...
  async detail(id: number) {
    ...
    if (!item) {
      throw NotFoundError("api.modules.action.not_found");
    }
    return item;
  }

  async update(id: number, payload: UpdateActionDTO) {
    const db = await this.repo.getExecutor();

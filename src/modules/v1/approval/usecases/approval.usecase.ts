 ...
 * src/modules/v1/approval/usecases/approval.usecase.ts
 */
...
export class ApprovalUseCase {
  ...
  async create(payload: CreateApprovalDTO) {
    ...
    await db.transaction(async (tx: unknown): Promise<ReturnType<typeof this.repo.create>> => {
      ...
      if (isExist) {
        throw ValidationError("api.common.validation_failed", {
          name: "api.modules.approval.already_exists",
        });
      }
  ...
  async detail(id: number) {
    ...
    if (!item) {
      throw NotFoundError("api.modules.approval.not_found");
    }
    return item;
  }

  async update(id: number, payload: UpdateApprovalDTO) {
    const db = await this.repo.getExecutor();

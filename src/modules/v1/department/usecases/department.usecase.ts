 ...
 * src/modules/v1/department/usecases/department.usecase.ts
 */

import { DepartmentRepository } from "@/modules/v1/department/repositories/department.repository";
...
export class DepartmentUseCase {
    ...
    await db.transaction(async (tx: unknown): Promise<ReturnType<typeof this.repo.create>> => {
      ...
      if (isExist) {
        throw ValidationError("api.common.validation_failed", {
          name: "api.modules.department.already_exists",
        });
      }
  ...
  async detail(id: number) {
    ...
    if (!item) {
      throw NotFoundError("api.modules.department.not_found");
    }
    return item;
  }
